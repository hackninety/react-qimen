...
| 符首落中五宫（taobi） | 值符宫的星是「天芮/天禽」并写，无法直接判定值符：符首地盘落中五时寄坤二，值符=天禽、值使=死门；由适配器显式处理（回归用例：2024-12-22 01:30 冬至阳遁4局） |
| 3meta 门宫关系 | `GatePressure` 取值为单字 `'迫'/'制'/'和'/'义'`，仅 `迫` 记为门迫标记，其余进 extras（曾误判所有非"无"值为门迫） |
| 晚子时（23:00-24:00） | 共享历法用 `getDayInGanZhi()`（子夜换日），与 jelly 默认 `same-day` 一致；各引擎内部自处理。跨引擎对照若在此边界出现差异属流派约定不同，非 bug |
| 真太阳时 | `utils/true-solar-time.ts`：真太阳时 = 输入钟表时间 − 时区偏移 + 经度×4分钟 + 均时差（Spencer 公式）；地点来源三选一：全国省市区经度库（`lib/cities.ts`，取自 react-taiyi）/ 浏览器时区代表城市 / 手动经度。启用后以修正后的时间调用引擎 |
| 导出（MD/JSON） | `utils/export.ts`：MD 紧凑（格局只列名称，省 token）、JSON 完整（含格局断语）；两者均附排盘口径（流派/定局法/引擎）与地理/时间上下文（地名、经度、时区、修正明细），供 AI 推理参照 |
| 静态知识库 | `lib/reference.ts`：门吉凶（开休生吉/景杜平/死惊伤凶）、九星别名与简义、八门主事、八神简义——引擎无关的公有口径，驱动九宫详解与断局指引（`utils/analysis.ts`）；具体断验以引擎输出的格局/标记为准 |
| UI 版式 | 参照旧版 react-qmdj（fork 自 qfdk/qimen）的单列卡片流：吸顶导航+主题切换 → 起局 → 基础信息（四柱竖卡）→ 九宫格（吉凶指示条/符使角标/水印宫数/旺衰角标）→ 断局指引 → 格局 → 九宫详解 → 导出&AI Prompt |
| 鸣法中宫门名 | qimen-mingfa 中宫的门输出为「中五」（鸣法体系的中门称谓），照引擎原始命名显示，不做改写 |
